...
# ==========================================
# Escolha do tipo de banco: 'sqlite' ou 'postgresql'
DB_TYPE = 'postgresql'  # Alterar para 'postgresql' após configurar
...
try:
    from config_postgres import POSTGRES_CONFIG, DATABASE_URL
except ImportError:
...
def init_db():
    ...
    # Adicionar coluna tipo_caixa se não existir (apenas para SQLite)
    if DB_TYPE == 'sqlite':
        try:
    ...
    # Adicionar coluna evento_id para vincular transações a eventos (apenas para SQLite)
    if DB_TYPE == 'sqlite':
        try:
            cursor.execute("ALTER TABLE transacoes ADD COLUMN evento_id INTEGER")
        except sqlite3.OperationalError:
            pass  # A coluna já existe
